...
sdt = setdatetime.SET_DATE_AND_TIME()
...
class SDL_DS3231():
    ...
    def _read_seconds(self):
        return _bcd_to_int(self._read(
            self._REG_SECONDS) & 0x7F)  # wipe out the oscillator on bit
    ...
    def write_all(self, seconds=None, minutes=None, hours=None, day=None,
        ...
        if hours is not None:
            ...
            self._write(self._REG_HOURS, _int_to_bcd(
                hours))  # not  | 0x40 according to datasheet
    ...
    def write_now(self):
        ...
        self.write_datetime(x)

    def set_datetime(self):
        yyyy = raw_input("Enter Year: ")
        mnt = raw_input("Enter Month: ")
        day = raw_input("Enter day: ")
        hr = raw_input("Enter hour:")
        min = raw_input("Enter minute:")
        sec = raw_input("Enter second:")
        # time_in_string = "8-10-2018 11:52:40"
        time_in_string = day + "-" + mnt + "-" + yyyy + " " + hr + ":" + min + ":" + sec
        time_in_datetime = datetime.strptime(time_in_string, "%d-%m-%Y %H:%M:%S")
        self.write_datetime(time_in_datetime)

    def set_datetime_through_serial(self):
        sdt.setDate()
        sdt.setTime()
        time_in_string = sdt.dateString + " " + sdt.timeString
        time_in_datetime = datetime.strptime(time_in_string, "%d-%m-%Y %H:%M:%S")
        self.write_datetime(time_in_datetime)